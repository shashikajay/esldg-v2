...
// WebTorrent Global Error Handler to prevent fatal crashes
client.on('error', (err) => {
...
db.serialize(() => {
    ...
    // Initialize default master admin if not exists
    db.get(`SELECT * FROM users WHERE username = ?`, [process.env.APP_USERNAME || 'admin'], async (err, row) => {
        if (!row) {
            try {
                ...
                console.error("[Database Error] Failed to generate admin hash:", hashErr);
            }
        }
    });

    // Resume previously active torrents on server boot
...
const requireAuth = (req, res, next) => req.session.loggedIn ? next() : res.status(401).json({ error: 'Unauthorized Access' });
const requireAdmin = (req, res, next) => (req.session.loggedIn && req.session.role === 'admin') ? next() : res.status(403).json({ error: 'Forbidden. Administrator privileges required.' });
...
app.post('/api/register', async (req, res) => {
    ...
    try {
        ...
        db.run(`INSERT INTO users (username, email, password, otp) VALUES (?, ?, ?, ?)`, [username, email, hash, otp], async function(err) {
            if (err) return res.status(400).json({ error: 'Username or Email is already registered.' });
            try { 
                await transporter.sendMail({ from: process.env.SMTP_USER, to: email, subject: 'ESLDG V2 System Verification', text: `Your verification code is: ${otp}` }); 
            } catch (mailErr) { 
                console.log(`[MAIL ERROR] Failed to send OTP to ${email}. Code: ${otp}`); 
            }
            logAction(username, 'Account provisioned');
...
app.post('/api/login', (req, res) => {
    ...
    db.get(`SELECT * FROM users WHERE username = ?`, [username], async (err, user) => {
        if (!user || !(await bcrypt.compare(password, user.password))) return res.status(401).json({ error: 'Invalid authentication credentials.' });
        if (user.is_verified === 0) return res.status(403).json({ error: 'Account requires email verification.', requiresOTP: true, email: user.email });
        
        req.session.loggedIn = true; 
        req.session.username = user.username; 
        req.session.role = user.role;
        logAction(user.username, 'Session authenticated');
...
app.post('/api/admin/user/:id/delete', requireAdmin, (req, res) => {
    db.get(`SELECT username FROM users WHERE id = ?`, [req.params.id], (err, user) => {
        if (user && user.username !== process.env.APP_USERNAME) {
            ...
            logAction(req.session.username, `Deleted user account: ${user.username}`);
            res.json({ success: true });
        } else {
            res.status(400).json({ error: 'Cannot purge the master administrator account.' });
        }
    });
});
...
// --- SEEDBOX & TORRENT PIPELINE ---
app.post('/api/add', requireAuth, (req, res) => {
    const { link } = req.body;
    const userDir = getUserDir(req.session.username);
    logAction(req.session.username, `Added remote link: ${link.substring(0, 30)}...`);
    ...
    if (link.startsWith('magnet:')) {
        ...
        res.json({ success: true, message: 'Magnet stream mounted successfully.' });
    } else if (link.startsWith('http')) {
        const fileName = path.basename(new URL(link).pathname) || `download_${Date.now()}`;
        axios({ url: link, method: 'GET', responseType: 'stream' }).then(response => {
            response.data.pipe(fs.createWriteStream(path.join(userDir, fileName)));
        }).catch(err => {
            console.error('[HTTP Download Error]', err.message);
        });
        res.json({ success: true, message: 'HTTP transfer pipeline established.' });
    } else {
...
app.post('/api/upload', requireAuth, upload.single('torrent'), (req, res) => {
    if (!req.file) return res.status(400).json({ error: 'Payload missing.' });
    const userDir = getUserDir(req.session.username);
    ...
    res.json({ success: true, message: 'Torrent file parsed and mounted.' });
});
...
app.post('/api/torrent/:hash/pause', requireAuth, (req, res) => {
    ...
    if (torrent) { 
        torrent.pause(); 
        db.run(`UPDATE torrents SET status = 'paused' WHERE infoHash = ?`, [req.params.hash]); 
    }
    res.json({ success: true });
});
...
app.post('/api/torrent/:hash/resume', requireAuth, (req, res) => {
    ...
    if (torrent) { 
        torrent.resume(); 
        db.run(`UPDATE torrents SET status = 'active' WHERE infoHash = ?`, [req.params.hash]); 
    }
    res.json({ success: true });
});
...
app.post('/api/torrent/:hash/cancel', requireAuth, (req, res) => {
    ...
    if (torrent) {
        ...
        torrent.destroy(() => { 
            if (deleteData && fs.existsSync(torrentPath)) fs.rmSync(torrentPath, { recursive: true, force: true }); 
        });
...
app.get('/api/files', requireAuth, (req, res) => {
    ...
    const files = fs.readdirSync(targetDir).map(file => {
        ...
        return { 
            name: file, 
            path: path.join(req.query.path || '', file).split(path.sep).join('/'), 
            size: stats.size, 
...
app.get('/api/download-zip', requireAuth, (req, res) => {
    ...
    if (!targetDir.startsWith(userDir) || !fs.existsSync(targetDir)) return res.status(403).json({ error: 'Access Denied' });
    ...
    try {
        ...
        // Memory-Optimized Archiver Logic
        const createArchive = archiver.default || archiver;
        const archive = createArchive('zip', {
            zlib: { level: 1 } // Low compression prioritizes RAM & CPU efficiency
        });
        ...
        if (!res.headersSent) res.status(500).send('Server Error generating ZIP archive.');
    }
});

app.post('/api/delete', requireAuth, (req, res) => {
    const fullPath = path.resolve(getUserDir(req.session.username), req.body.path);
    if (fullPath.startsWith(getUserDir(req.session.username)) && fs.existsSync(fullPath)) {
...
app.get('/api/storage', requireAuth, (req, res) => {
    exec(`df -B1 ${DOWNLOAD_DIR}`, (error, stdout) => {
        const fallbackMax = 5 * 1024 * 1024 * 1024; // 5GB fallback
        let maxBytes = fallbackMax;
        
        if (!error && stdout.split('\n').length > 1) {
            maxBytes = parseInt(stdout.trim().split('\n')[1].replace(/\s+/g, ' ').split(' ')[1], 10);
        }
        ...
        const usedBytes = getDirSize(DOWNLOAD_DIR);
        res.json({ used: usedBytes, max: maxBytes });
    });
});

// --- MEDIA ENGINE & SUBTITLES ---
app.get('/api/subtitles', requireAuth, (req, res) => {
    ...
    const subtitles = fs.readdirSync(dir)
        .filter(f => f.endsWith('.vtt') || f.endsWith('.srt'))
        .map(f => ({
            name: f, 
            src: `/downloads/${req.session.username}/${encodeURIComponent(path.join(path.dirname(req.query.path), f)).replace(/%2F/g, '/')}`, 
            isMatch: f.includes(baseName)
        }));
...
app.get('/stream/*', requireAuth, (req, res) => {
    ...
    if (!filePath.startsWith(userDir) || !fs.existsSync(filePath)) return res.status(404).send('Resource not found.');
...
process.on('SIGINT', () => {
    ...
    client.destroy(() => {
        db.close(() => {
            console.log('[System] Database connection closed. Exiting process.');
            process.exit(0);
        });
    });
});
...
app.listen(PORT, () => console.log(`[ESLDG V2] System Matrix Online on Port ${PORT}`));